...
#define NUM_ROWS    25
#define ATTRIB      0x7
#define VIDEO_CONTENT(x,y) *(uint8_t *)(video_mem + ((NUM_COLS * y + x) << 1))
 ...
 * Inputs: void
 * Return Value: none
 * Function: Clears 1 character, move curosr */
void bksp(void) {
    screen_x --;
    if (screen_x < 0) {
        screen_y --;
        ...
        if (screen_y < 0) {
            ...
            while (screen_x > 1 && VIDEO_CONTENT(screen_x - 1, screen_y) == ' ')
                screen_x --;
        }
    }
    VIDEO_CONTENT(screen_x, screen_y) = ' ';
...
void putc(uint8_t c) {
    ...
    if (screen_y == NUM_ROWS) {
        ...
        for (i = 0; i < (NUM_ROWS - 1) * NUM_COLS; i++) {
            ...
            *(uint8_t *)(video_mem + (i << 1) + 1) =  *(uint8_t *)(video_mem + (j << 1) + 1);
        }
...
/* void putc at(uint8_t c);
 * Inputs: uint_8* c = character to print, x y cords
 * Return Value: void
 *  Function: Output a character to the console at x, y */
